...
export default function CreateFormPage() {
  ...
  const preselectedChecklistId = searchParams.get('checklistId');
  const { showToast } = useToast();

  const [loading, setLoading] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [checklists, setChecklists] = useState<{ id: number; tanggal: string; status: string }[]>([]);

  const [checklistId, setChecklistId] = useState(preselectedChecklistId || '');
  ...
  const removeFodImage = (index: number) => {
    setFodImages(prev => prev.filter((_, i) => i !== index));
  };

  const updateFodCaption = (index: number, caption: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        showToast('success', 'Form inspeksi berhasil dibuat');
        router.push(`/dashboard/form/${data.id}`);
      } else {
        const err = await res.json();
        showToast('error', err.error || 'Gagal membuat form inspeksi');
      }
    } catch {
    ...
    return (
      <table className="w-full text-sm border-collapse">
        ...
        <tbody>
          {items.map(item => {
            if (item.level === 0) {
              return null; // Category header is shown as section title
            }
            if (item.level === 1) {
              return (
                <tr key={item.id} className="bg-blue-50">
                  <td className="border border-gray-300 px-3 py-2 font-semibold" colSpan={9}>{item.label}</td>
                </tr>
              );
            }
            counter++;
            ...
            return (
              <tr key={item.id} className="hover:bg-gray-50">
                ...
                <td className="border border-gray-300 px-3 py-2 text-center">
                  <input type="checkbox" checked={d.ada} onChange={e => updateInspection(sectionKey, item.id, 'ada', e.target.checked)} className="w-4 h-4 accent-blue-600" />
                </td>
                <td className="border border-gray-300 px-3 py-2 text-center">
                  <input type="checkbox" checked={d.tidakAda} onChange={e => updateInspection(sectionKey, item.id, 'tidakAda', e.target.checked)} className="w-4 h-4 accent-blue-600" />
                </td>
                <td className="border border-gray-300 px-3 py-2 text-center">
                  <input type="checkbox" checked={d.baik} onChange={e => updateInspection(sectionKey, item.id, 'baik', e.target.checked)} className="w-4 h-4 accent-blue-600" />
                </td>
                <td className="border border-gray-300 px-3 py-2 text-center">
                  <input type="checkbox" checked={d.kurangBaik} onChange={e => updateInspection(sectionKey, item.id, 'kurangBaik', e.target.checked)} className="w-4 h-4 accent-blue-600" />
                </td>
                <td className="border border-gray-300 px-3 py-2">
                  <input type="text" value={d.upaya} onChange={e => updateInspection(sectionKey, item.id, 'upaya', e.target.value)} className="w-full border border-gray-200 rounded px-2 py-1 text-xs" placeholder="-" />
                </td>
                <td className="border border-gray-300 px-3 py-2">
                  <input type="text" value={d.tindakLanjut} onChange={e => updateInspection(sectionKey, item.id, 'tindakLanjut', e.target.value)} className="w-full border border-gray-200 rounded px-2 py-1 text-xs" placeholder="-" />
                </td>
                <td className="border border-gray-300 px-3 py-2">
                  <input type="text" value={d.keterangan} onChange={e => updateInspection(sectionKey, item.id, 'keterangan', e.target.value)} className="w-full border border-gray-200 rounded px-2 py-1 text-xs" placeholder="-" />
                </td>
              </tr>
            );
          })}
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Buat Form Inspeksi (FORM.01)</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Referensi Checklist</h2>
          <select value={checklistId} onChange={e => setChecklistId(e.target.value)} required className="w-full border border-gray-300 rounded-lg px-4 py-2.5">
            <option value="">Pilih Checklist yang sudah diapprove</option>
            {checklists.map(cl => (
              <option key={cl.id} value={cl.id}>CL #{cl.id} - {new Date(cl.tanggal).toLocaleDateString('id-ID')}</option>
            ))}
          </select>
          {checklists.length === 0 && <p className="text-sm text-orange-600 mt-2">⚠️ Belum ada checklist yang diapprove. Buat dan approve checklist terlebih dahulu.</p>}
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input type="text" value={hari} onChange={e => setHari(e.target.value)} placeholder="Senin" required className="w-full border border-gray-300 rounded-lg px-4 py-2.5" />
            </div>
            <div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={petugasNama1} onChange={e => setPetugasNama1(e.target.value)} placeholder="Nama petugas 1" className="w-full border border-gray-300 rounded-lg px-4 py-2.5" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Petugas 2</label>
              <input type="text" value={petugasNama2} onChange={e => setPetugasNama2(e.target.value)} placeholder="Nama petugas 2" className="w-full border border-gray-300 rounded-lg px-4 py-2.5" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">Upload foto temuan benda asing di area inspeksi</p>
            ...
            <label className={`flex items-center justify-center border-2 border-dashed rounded-lg px-6 py-8 cursor-pointer transition-colors ${uploading ? 'border-gray-300 bg-gray-50' : 'border-blue-300 hover:border-blue-400 hover:bg-blue-50'}`}>
              <div className="text-center">
                {uploading ? (
                  ...
                  <><span className="text-3xl">📷</span><p className="text-sm text-gray-600 mt-2">Klik untuk upload foto FOD</p><p className="text-xs text-gray-400">JPG, PNG (max 10MB)</p></>
                )}
              </div>
        ...
        <div className="flex items-center justify-between bg-white rounded-xl border border-gray-200 p-6">
          <button type="button" onClick={() => router.back()} className="px-6 py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">Batal</button>
          <button type="submit" disabled={loading} className="px-8 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 font-medium">
            {loading ? 'Menyimpan...' : 'Simpan Form Inspeksi'}
          </button>
        </div>
      </form>
    </div>
